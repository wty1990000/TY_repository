...
*/
{
	...
	for (int i = 0; i < m_iNumberY; i++)
	{
		for (int j = 0; j < m_iNumberX; j++)
		{
			...
			m_dTemp = sqrt(m_dModf * m_dModg) * float(m_iFFTSubW * m_iFFTSubH); //parameter for normalization
			...
			{
				m_iCorrPeakX[i*m_iNumberX+j] -= m_iFFTSubW;
			}
			if (m_iCorrPeakY[i*m_iNumberX+j] > m_iSubsetY)
			{
				m_iCorrPeakY[i*m_iNumberX+j] -= m_iFFTSubH;
			}
