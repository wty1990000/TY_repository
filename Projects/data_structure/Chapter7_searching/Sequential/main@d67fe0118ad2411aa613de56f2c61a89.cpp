...
Error_code sequatial_search(const CList<Record>&the_list,const Key &target, int &position)
{
	int s = the_list.size();
...
void print_out(const char *x, const double &elapsed_time, const int &comparison, const int &search)
{
	cout<<x<<" Findings using time: "<<elapsed_time
		<<"s to finish "<<comparison<<" comparisons"
