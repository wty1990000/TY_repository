		...
		float d2=0;
	 	...
	 	for(size_t k=0;k<total_points;k++) {
			...
			for (matrix_iterator A = Abegin; A != Aend;++A) {
				unsigned int j   = A->first;
				glm::mat3& A_ij  = A->second;
				float prevx = prevn[j].x;
				float prevy = prevn[j].y;
				float prevz = prevn[j].z;
